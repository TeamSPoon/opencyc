...
public class CycLeaseManager extends Thread {
  ...
  public void run() {
    ...
    while (!cycAccess.isClosed()) {
      ...
      try {
        ...
        logger.fine("Cyc communications timeout encountered when attempting "
          + "to renew the API services lease.\n" + toe.getMessage());
        ...
        isLeaseRequestPending = false;
        logger.fine("Cyc communications error encountered when attempting " 
          + "to renew the API services lease.\n" + e.getMessage());
      ...
      if (results.equals("api services lease denied")) {
        ...
        assert results.startsWith("api services lease granted by ");
        final String currentImageID = results.substring(30);
  ...
  public class CycLeaseEventObject extends EventObject {
    ...
    public CycLeaseManagerReason getReason() { return cycLeaseManagerReason; }
  ...
  private void notifyListeners(final CycLeaseManagerReason cycLeaseManagerReason) {
    ...
    assert listeners != null : "listeners must not be null";
    assert cycAccess != null : "cycAccess must not be null";
    if ((cycLeaseManagerReason == CycLeaseManager.CYC_COMMUNICATION_ERROR)
        || (cycLeaseManagerReason == CycLeaseManager.CYC_DENIES_THE_LEASE_REQUEST)
        || (cycLeaseManagerReason == CycLeaseManager.CYC_DOES_NOT_RESPOND_TO_LEASE_REQUEST)) {
      hasValidLease = false;
    } else {
      hasValidLease = true;
    }
  ...
  private boolean isLeaseRequestPending = false;
  ...
  public static void main (String[] args) {
    try {
      final CycAccess cycAccess = new CycAccess("aruba.cyc.com", 3640);
//      cycAccess.traceOn();
      for (int i = 0; i < 2; i++) {
        Thread.sleep(20000);
        final String script = "(sleep 20)";
